...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, args, word_freq, langs, char_vocab_size, word_vocab_size, tagset_size):
    ...
    def forward(self, words, word_idxs=None, lang=None, test=False):
        ...
        if self.sum_word_char:
            self.word_freq[self.word_vocab_size-1] = 0
        ...
        if self.model_type=="specific" and lang:
            ...
            if self.langs.index(lang)==0:
                tag_space = F.tanh(self.hidden2tag_1(lstm_out.view(char_embs.size(0), -1)))
            else:
                tag_space = F.tanh(self.hidden2tag_2(lstm_out.view(char_embs.size(0), -1)))
            ...
            tag_space_1 = F.tanh(self.hidden2tag_1(lstm_out.view(char_embs.size(0), -1)))
            tag_space_2 = F.tanh(self.hidden2tag_2(lstm_out.view(char_embs.size(0), -1)))
